...
# Define report date
report_date <- as.Date("2026-06-30")
```
...
1. Describe the **trends in Pharmacy First service use** over time, including variation by demographic and regional subgroups
...
**Figure 1**. Monthly count of patients with a Pharmacy First consultation code in their GP records in OpenSAFELY-TPP, broken down by individual Pharmacy First consultation codes. 
The dashed vertical line indicates the launch of Pharmacy First on 31st January 2024. CP = Community Pharmacy.
...
df_pf_consultations_extended <- generate_pf_consultation_counts_extended(df_measures, report_date)

write_csv(df_pf_consultations_extended, here("output", "report", "data", "df_pf_consultations_updated.csv"))
...
plot_pf_consultations <- df_pf_consultations_extended |>
  ...
  scale_x_date(
    breaks = sort(unique(c(
      seq(
        min(df_pf_consultations_extended$interval_start, na.rm = TRUE),
        max(df_pf_consultations_extended$interval_start, na.rm = TRUE),
        by = "2 months"
      ),
      max(df_pf_consultations_extended$interval_start, na.rm = TRUE)
    ))),
...
ggsave(here("output", "report", "figures", "figure1_updated.png"), plot_pf_consultations, width = 10, height = 8, create.dir = TRUE)
```
...
df_pf_consultations_breakdown <- df_measures %>%
...
write_csv(df_pf_consultations_breakdown, here("output", "report", "data", "df_pf_consultations_breakdown_updated.csv"))
ggsave(here("output", "report", "figures", "figure2sex_updated.png"), pf_sex, width = 10, height = 4)
```
...
pf_age
ggsave(here("output", "report", "figures", "figure3age_updated.png"), pf_age, width = 10, height = 4)
```
...
pf_imd
ggsave(here("output", "report", "figures", "figure4imd_updated.png"), pf_imd, width = 10, height = 4)
```
...
pf_region
ggsave(here("output", "report", "figures", "figure5region_updated.png"), pf_region, width = 10, height = 4)
```
...
pf_ethnicity
ggsave(here("output", "report", "figures", "figure6ethnicity_updated.png"), pf_ethnicity, width = 10, height = 15)
```
...
df_pf_descriptive_stats <- generate_linkage_dataset(df_descriptive_stats, df_pf_consultations, report_date)

write_csv(df_pf_descriptive_stats, here("output", "report", "data", "df_pf_descriptive_stats_updated.csv"))
...
plot_pf_med_condition_linkage <- df_pf_descriptive_stats |>
  ...
  scale_x_date(
    breaks = sort(unique(c(
      seq(
        min(df_pf_descriptive_stats$interval_start, na.rm = TRUE),
        max(df_pf_descriptive_stats$interval_start, na.rm = TRUE),
        by = "2 months"
      ),
      max(df_pf_descriptive_stats$interval_start, na.rm = TRUE)
    ))),
    labels = scales::label_date_short(),
    # Leave enough room for the percentage labels centred on the final month.
...
ggsave(here("output", "report", "figures", "figure7_updated.png"), plot_pf_med_condition_linkage, width = 10, height = 4)
```

## Summary of clinical conditions recorded in Pharmacy First consultations

Pharmacy First consultations involving a clinical condition, as specified by NHS England, are shown here.
This highlights which pathways were most commonly used between 2024-01-31 and `r report_date`.

**Figure 8**. Monthly count of patients with a Pharmacy First consultation code in OpenSAFELY-TPP broken down by clinical condition.

```{r plot-pf-consultations-by-clinical-pathway, message=FALSE, warning=FALSE, fig.height=15, fig.width=10}
pf_clinical_pathways <- df_measures %>%
  filter(between(interval_start, as.Date("2024-02-01"), report_date)) |>
...
df_pf_clinical_pathways <- df_measures %>% 
  filter(measure_desc == "clinical_condition") %>% 
  filter(is.na(group_by))

write_csv(df_pf_clinical_pathways, here("output", "report", "data", "df_pf_clinical_pathways_updated.csv"))
ggsave(here("output", "report", "figures", "figure8clinicalpathways_updated.png"), pf_clinical_pathways, width = 10, height = 15)
```
...
**Table 1**. Top ten medications recorded in Pharmacy First consultations between 2024-01-31 and `r report_date`.

```{r, message=FALSE, warning=FALSE}
# Creates table of top 10 med grouped by whether they are included in PF codelist or not
df_pf_and_non_pf_med_counts <- generate_meds_dataset(df_consultation_med_counts, report_date)

write_csv(df_pf_and_non_pf_med_counts, here("output", "report", "data", "df_pf_and_non_pf_med_counts_updated.csv"))
...
Monthly counts of consultations were summarised by clinical condition to illustrate changes in activity over time and variation between conditions.
