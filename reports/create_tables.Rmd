...
# Using the 'term' from the first instance of bnf_code where dmd_type is VMP
pf_med_code_desc <- here("codelists", codelist_file_names) |>
  ...
  bind_rows() |>
  select(code, bnf_code, dmd_type, term) |>
  filter(!is.na(bnf_code)) %>% 
  group_by(bnf_code) %>%
  summarise(
    code = code,
    term = first(term[dmd_type == "VMP"]),
    dmd_type,
  ) %>%
  distinct()
...
clinical_pathways_table_by_sex <- population_csv %>%
