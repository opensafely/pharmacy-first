...
A key element of the recovery plan is the Pharmacy First scheme, which launched on 31st January 2024<!-- [@NHSEngland2024] -->.
This includes a new clinical pathways service<!-- [@NHSEngland2023] -->, which allows community pharmacists to follow specified clinical pathways to assess for and manage seven common conditions (sinusitis, acute pharyngitis, acute otitis media, infected insect bites, impetigo, herpes zoster, and uncomplicated urinary tract infections in women), including supplying NHS funded prescription only medications where appropriate<!--  [@NHSEngland2024] -->. 
...
The aims of this regular updating report are to:
...
1. Describe and compare Pharmacy First activity in GP records accessible via OpenSAFELY-TPP with publicly available data;
...
To contextualise the data used in this report, we compared overall consultation counts in OpenSAFELY-TPP with publicly available NHS BSA data, which captures 100% of Pharmacy First consultation activity that was reimbursed by NHS England (15). This comparison focused on aggregate consultation counts and the proportion of Pharmacy First consultations recorded in OpenSAFELY-TPP relative to NHS BSA over time. We also compared counts of Pharmacy First consultations by clinical condition to assess the representativeness of our population.
Completeness of Pharmacy First consultations in GP records 
We examined the recording of clinical information recorded with Pharmacy First consultations by extracting clinical events and medications linked to each consultation using the consultation identifier. The code example in Box 1 illustrates how we query all clinical events and medications with the same consultation identifier as a Pharmacy First Consultation. Completeness was assessed to determine the extent to which key clinical information- specifically conditions and medications - is consistently recorded in structured data. Consultations were categorised in three mutually exclusive groups: (i) those with a recorded clinical condition from one of the seven pre-specified clinical conditions; (ii) those with a prescribed Pharmacy first medication; and (iii) those with both a recorded Pharmacy first-specific clinical condition and medication.
Monthly proportions were calculated to assess changes in recording patterns over time.

### Summary of clinical conditions and medications recorded with Pharmacy First consultations

We identified Pharmacy First clinical conditions by extracting all SNOMED CT codes recorded during Pharmacy First consultations that matched the seven NHS England-defined clinical conditions. In order to support the wider evaluation of the implementation of Pharmacy First and to capture the full range of recorded consultations the inclusion and exclusion criteria associated with each pathway were not applied. Counts of clinical conditions were grouped by pathway and broken down by patient sex and Index of Multiple Deprivation (IMD) to get a better understanding of the population using the service for each condition. To compare clinical condition recording between data sources, we calculated the percentage of Pharmacy First consultations attributed to each of the seven NHS England-specified clinical conditions in OpenSAFELY-TPP and NHS BSA data.
...
This table compares the demographic breakdown of patients receiving a Pharmacy First consultation to the overall OpenSAFELY-TPP population.
...
We compared the number of Pharmacy First consultations recorded in OpenSAFELY-TPP to NHS BSA data. NHS BSA captures 100% of Pharmacy First activity across England, while OpenSAFELY-TPP represents only around 40% of the population. This figure highlights the difference in counts and shows what proportion of nationally recorded consultations are captured in OpenSAFELY-TPP GP records.

**Figure 2**. Monthly counts and ratio of patients with a Pharmacy First consultation code in OpenSAFELY-TPP and the NHS BSA comparison dataset.
...
**Figure 3**. Monthly count of patients with a Pharmacy First consultation code in OpenSAFELY-TPP broken down by sex.
...
**Figure 4**. Monthly count of patients with a Pharmacy First consultation code in OpenSAFELY-TPP broken down by age bands.
...
**Figure 5**. Monthly count of patients with a Pharmacy First consultation code in OpenSAFELY-TPP broken down by IMD.
...
**Figure 6**. Monthly count of patients with a Pharmacy First consultation code in OpenSAFELY-TPP broken down by region.
...
**Figure 7**. Monthly count of patients with a Pharmacy First consultation code in OpenSAFELY-TPP broken down by ethnicity.
...
## Completeness of recorded Pharmacy First consultations in OpenSAFELY-TPP
