...
The data used comprised coded diagnoses, medications and physiological parameters.
...
To reduce the risk of re-identification, all counts smaller or equal 7 were redacted, then all counts were rounded to the nearest 5 before calculating rates.
...
To contextualise the data used in this report, we compared overall consultation counts in OpenSAFELY-TPP with publicly available NHS BSA data, which captures 100% of Pharmacy First consultation activity that was reimbursed by NHS England (15).
...
We also compared counts of Pharmacy First consultations by clinical condition to assess the representativeness of our population.
Completeness of Pharmacy First consultations in GP records 
We examined the recording of clinical information recorded with Pharmacy First consultations by extracting clinical events and medications linked to each consultation using the consultation identifier.
The code example in Box 1 illustrates how we query all clinical events and medications with the same consultation identifier as a Pharmacy First Consultation.
Completeness was assessed to determine the extent to which key clinical information- specifically conditions and medications - is consistently recorded in structured data.
...
In order to support the wider evaluation of the implementation of Pharmacy First and to capture the full range of recorded consultations the inclusion and exclusion criteria associated with each pathway were not applied.
Counts of clinical conditions were grouped by pathway and broken down by patient sex and Index of Multiple Deprivation (IMD) to get a better understanding of the population using the service for each condition.
To compare clinical condition recording between data sources, we calculated the percentage of Pharmacy First consultations attributed to each of the seven NHS England-specified clinical conditions in OpenSAFELY-TPP and NHS BSA data.
