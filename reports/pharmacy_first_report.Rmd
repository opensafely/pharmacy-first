...
In response to this, NHS England announced a recovery plan to "*make it easier and quicker for patients to get the help they need from 
primary care*"<!-- [@NHSEngland2023a] -->.

A key element of the recovery plan is the Pharmacy First scheme, which launched on 31st January 2024<!-- [@NHSEngland2024] -->.
This includes a new clinical pathways service<!-- [@NHSEngland2023] -->, which allows community pharmacists to follow specified
clinical pathways to assess for and manage seven common conditions (Sinusitis, Sore throat, Earache, Infected insect bite, 
Impetigo, Shingles, and Uncomplicated urinary tract infections in women), including supplying NHS funded prescription only 
medications where appropriate<!--  [@NHSEngland2024] -->. 

As part of efforts to improve and simplify information sharing between general practice and community pharmacy 
GP Connect: Update Record has been created, allowing registered community pharmacy professionals to send summaries in a 
structured format from Pharmacy First consultations, using pre-specified SNOMED CT codes, directly into general practice 
workflow<!--  [@NHSEngland2023b] -->.
However, the roll out of Update Record has been gradual due to the involvement of multiple community pharmacy and GP system 
suppliers, with each being ready at different stages<!--  [@Colivicchi2024; @CommunityPharmacyEngland2024] -->.

The aims of this report are to:

1. describe the patient population using the Pharmacy First Clinical Pathway service, 
including demographic and regional subgroups
2. describe the clinical conditions and medications being treated/supplied under Pharmacy First Clinical Pathways and 
recorded in GP records
3. compare Pharmacy First activity in GP records accessible via OpenSAFELY-TPP with publicly available data
...
Primary care records managed by the GP software provider TPP were accessed through OpenSAFELY (https://www.opensafely.org/), a secure analytics 
platform for EHRs built during the COVID-19 pandemic.
...
The data used comprised coded diagnoses, medications and physiological parameters. Data from individuals who opted out of NHS data sharing with their 
GP surgery (Type 1 Opt-out) were excluded and free-text data were also not included.

### NHS BSA comparison data
To explore the accuracy and completeness of the data used in this study, we compared Pharmacy First consultation counts and trends observed in 
OpenSAFELY-TPP against equivalent metrics reported by NHS Business Services Authority (NHS BSA), which provides publically available records of 
dispensing activity and service usage.
We used the Pharmacy and appliance contractor dispensing dataset which includes monthly counts of Pharmacy First Clinical 
Pathways<!--  [@NHSBusinessServiceAuthority2025] -->.
...
Using the GP Connect: Update Record technical documentation<!--  [@NHSEngland2024b] --> and OpenCodelists (https://www.opencodelists.org/), 
we identified three codes associated with Pharmacy First consultations (see Table A1 in Supplementary Materials).

### Pharmacy First clinical conditions

The Pharmacy First service includes the supply of medication for the following seven clinical conditions: acute otitis media, impetigo, 
infected insect bites, shingles, sinusitis, sore throat, and uncomplicated urinary tract infections<!--  [@NHSEngland2024] -->.
...
Code for data management and analysis, as well as codelists, are available for review and reuse under the MIT open license 
at https://github.com/opensafely/pharmacy-first/.
...
```
**Table 1**. Cohort description for all patients with at least one Pharmacy First consultation between `r min(df_measures$interval_start)` 
and `r max(df_measures$interval_end)`.
...
df_pf_descriptive_stats <- df_pf_descriptive_stats %>%
  ...
  mutate(
    ratio_pct = numerator / pf_consultation_total,
    cumulative_ratio_exclusive = case_when(interval_start == max(df_pf_descriptive_stats$interval_start) ~ cumsum(ratio_pct), TRUE ~ NA)
  )
...
plot_pf_med_condition_linkage <- df_pf_descriptive_stats |>
  ...
  geom_label(
    aes(
      ...
      label = ifelse(is.na(ratio_pct), "", scales::percent(ratio_pct, accuracy = .1)),
    ),
    vjust = -0.05,
    size = 3.5,
    show.legend = FALSE,
