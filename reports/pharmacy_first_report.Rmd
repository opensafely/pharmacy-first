...
```
**Table 1**. Description of patient population with at least one Pharmacy First consultation code (n = `r (n_pf_total)`) and total population in OpenSAFELY-TPP (N = `r (n_tpp_total)`) between `r min(df_measures$interval_start)` and `r max(df_measures$interval_end)`.
