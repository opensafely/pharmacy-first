...
This OpenSAFELY report presents analyses of the Pharmacy First service during the first year of its operation using data from OpenSAFELY-TPP. 
It is intended to support policymakers and clinicians in evaluating and improving service delivery, based on first-year findings.
...
# Create new "data" folder in "output" subdirectory
fs::dir_create(here("output", "report", "data"))

# Create a data frame with headers but no data
empty_df <- tibble(col1 = character(), col2 = integer())

# Only write_csv and calculate pf/tpp totals if df_demographics_table.csv is not NULL
if (!is.null(df_demographics_table)) {
  write_csv(df_demographics_table, here("output", "report", "data", "df_demographics_table.csv"))

  n_pf_total <- df_demographics_table %>%
    filter(category == "Sex") %>%
    summarise(total = sum(pharmacy_first_n)) %>%
    pull(total)

  n_tpp_total <- df_demographics_table %>%
    filter(category == "Sex") %>%
    summarise(total = sum(tpp_n)) %>%
    pull(total)

  n_pf_total <- format(n_pf_total, big.mark = ",", scientific = F)
  n_tpp_total <- format(n_tpp_total, big.mark = ",", scientific = F)
} else {
  message("Creating empty csv: df_demographics_table is NULL")
  write_csv(empty_df, here("output", "report", "data", "df_demographics_table.csv"))
  n_pf_total <- "Unavailable"
  n_tpp_total <- "Unavailable"
}
```
**Table 1**. Description of patient population with at least one Pharmacy First consultation code (n = `r (n_pf_total)`) and total population in OpenSAFELY-TPP (N = `r (n_tpp_total)`) between 2024-01-31 and 2025-01-30.
...
write_csv(df_pf_consultations, here("output", "report", "data", "df_pf_consultations.csv"))
...
This highlights which pathways were most commonly used between 2024-01-31 and 2025-01-30.
