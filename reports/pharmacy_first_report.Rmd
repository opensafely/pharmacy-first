...
source(here::here("lib", "functions", "function_get_validation_data.R"))
...
df_bsa_validation$count = df_bsa_validation$count * 0.4

# Our data for clinical conditions into a tidy df
...
df_validation_condition_counts <- bind_rows(df_opensafely, df_bsa_validation)

# Round the 'count' column to 0 decimal places (nearest whole number)
df_validation_condition_counts$count <- as.numeric(df_validation_condition_counts$count)
df_validation_condition_counts$count <- round(df_validation_condition_counts$count, digits = 0)
...
df_pivoted <- df_pivoted %>%
  ...
  summarise(across(everything(), sum, na.rm = TRUE), .groups = "drop")  # Sum the counts
