...
selected_med_events = get_consultationid_events(
    medications, pharmacy_first_ids
    ).where(
...
has_pf_consultation = selected_clinical_events.where(
    selected_clinical_events.snomedct_code.is_in(
...
has_pf_medication = selected_med_events.where(
    selected_med_events.dmd_code.is_in(
